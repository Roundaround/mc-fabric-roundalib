...
/**
 * {@link ConfigSerializer} implementation for Java {@code .properties} format.
 * Entries are stored flat with dotted keys ({@code group.key = value}).
 * <p>
 * To disambiguate types, string values are double-quoted in the file
 * ({@code key = "hello"}). List values are encoded as inline JSON arrays
 * ({@code key = [1, 2]}). Integers, floats, and booleans are written as-is.
 * <p>
