...
public class FloatSliderControl extends AbstractClickableControlWidget<FloatConfigOption> {
  ...
  public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
    ...
    int step = configOption.getStep();
    int scaledValue = Math.round(getScaledValue() * step);
    ...
    setScaledValue(scaledValue / (float) step);
    return true;
  }
  ...
  private void setScaledValue(float value) {
    float min = configOption.getMinValue().orElse(0f).floatValue();
    float max = configOption.getMaxValue().orElse(1f).floatValue();

    configOption.setValue(MathHelper.clamp(value * (max - min) + min, min, max));
  }

  private float getScaledValue() {
    float value = configOption.getValue();
    float min = configOption.getMinValue().orElse(0f).floatValue();
    float max = configOption.getMaxValue().orElse(1f).floatValue();
