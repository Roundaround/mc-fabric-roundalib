...
public abstract class ModRecipesProvider extends RecipesProvider {
    ...
    public void run(DataCache cache) {
        ...
        generateRecipes((recipeJsonProvider) -> {
            ...
            if (!set.add(identifier)) {
                ...
                saveRecipe(cache, recipe, path.resolve(
                        "data/" + this.modId + "/recipes/" + identifier.getPath() + ".json"));

                JsonObject advancement = recipeJsonProvider.toAdvancementJson();
                if (advancement != null) {
                    saveRecipeAdvancement(cache, advancement, path.resolve(
                            "data/" + this.modId + "/advancements/" + recipeJsonProvider.getAdvancementId().getPath() +
                                    ".json"));
                }
            }
        });
    ...
    public static void offerSmelting(Consumer<RecipeJsonProvider> exporter, List<ItemConvertible> inputs, ItemConvertible output, float experience, int cookingTime, String group) {
        RecipesProvider.offerSmelting(exporter, inputs, output, experience, cookingTime, group);
    }

    public static void offerBlasting(Consumer<RecipeJsonProvider> exporter, List<ItemConvertible> inputs, ItemConvertible output, float experience, int cookingTime, String group) {
    ...
    public static void offerStonecuttingRecipeAndSingularizeBricks(Consumer<RecipeJsonProvider> exporter, ItemConvertible output, ItemConvertible input, int count) {
    ...
    public static InventoryChangedCriterion.Conditions conditionsFromItemPredicates(ItemPredicate... items) {
