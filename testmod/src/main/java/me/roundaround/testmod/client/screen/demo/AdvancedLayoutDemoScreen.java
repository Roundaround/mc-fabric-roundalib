...
public class AdvancedLayoutDemoScreen extends Screen implements DemoScreen {
  ...
  protected void init() {
    ...
    LinearLayoutWidget leftPane = body.add(LinearLayoutWidget.vertical((self) -> {
      Divider divider = new Divider(body.getWidth() - body.getSpacing(), 2);
      self.setDimensions(divider.nextInt(), body.getHeight());
    }).spacing(GuiUtil.PADDING));
    leftPane.getMainPositioner().alignRight();

    LinearLayoutWidget searchRow =
        leftPane.add(LinearLayoutWidget.horizontal((self) -> self.setDimensions(
            leftPane.getWidth() - GuiUtil.PADDING, BUTTON_HEIGHT)).spacing(GuiUtil.PADDING));
    ...
    LinearLayoutWidget rightPane = body.add(LinearLayoutWidget.vertical((self) -> {
      Divider divider = new Divider(body.getWidth() - body.getSpacing(), 2);
      divider.skip(1);
      self.setDimensions(divider.nextInt(), body.getHeight());
    }).spacing(GuiUtil.PADDING));
    ...
    controlsRow.getMainPositioner().alignHorizontalCenter().alignVerticalCenter();
    ...
    controlsRow.add(LabelWidget.builder(this.textRenderer,
            ...
            .build(),
        (parent, self) -> self.setWidth(
            rightPane.getWidth() - 2 * (2 * GuiUtil.PADDING + IconButtonWidget.SIZE_V)));
  ...
  public static class PlaceholderListWidget
      extends NarratableEntryListWidget<PlaceholderListWidget.Entry> {
